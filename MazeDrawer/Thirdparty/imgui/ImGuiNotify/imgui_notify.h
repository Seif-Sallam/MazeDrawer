...
#define NOTIFY_FADE_IN_OUT_TIME 50	  // Fade in and out duration
...
public:
	inline void set_title(const char *format, ...)
	{
		if (format)
		{
			va_list args;
	...
	inline void set_type(const ImGuiToastType &type)
	{
...
public:
	...
	inline const char *get_default_title()
	{
	...
	inline ImGuiToastType get_type()
	{
		return this->type;
	};

	inline ImVec4 get_color()
	{
		switch (this->type)
		{
		case ImGuiToastType_None:
	...
	inline auto get_icon() -> const char *
	{
		switch (this->type)
		{
		case ImGuiToastType_None:
	...
	{
		auto value = GetTickCount64() - this->creation_time;
		value = value - 1 + 1;
		return value;
	}

	inline ImGuiToastPhase get_phase()
	{
