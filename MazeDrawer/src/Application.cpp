...
void Application::ImGuiLayer()
{
	...
	if (CreateImGuiWindow("TestWindow"))
	{
		...
		{
			SaveData(std::string(m_FileName) + ".txt", m_SaveUntil, m_NumberOfCells, m_Cells);
			ImGui::InsertNotification({ImGuiToastType_Success, 100, "File was saved successfully to the path: %s", (std::string(m_FileName) + ".txt").c_str()});
		}
		...
		if (ImGui::Button("Load Map") && !loadMap)
		{
			loadMap = true;
		}
		ImGui::InputText("Save File Path", m_FileName, 1024);

		if (loadMap)
		{
			// Our filters are just txt files or maybe ini files
			const char *filters = "Text files (*.txt) {.txt}";
			// We don't want the "choosing a file window" to dock into anything.
			ImGuiWindowFlags flags = ImGuiWindowFlags_None;
			flags |= ImGuiWindowFlags_NoDocking;

			std::string key = "Dialog";
			ImGuiFileDialog::Instance()->OpenDialog(key, "Choose a map to open", filters, ".");
			if (ImGuiFileDialog::Instance()->Display(key, flags))
			{
				if (ImGuiFileDialog::Instance()->IsOk())
				{
					if (ImGuiFileDialog::Instance()->GetCurrentFileName() != "")
					{
...
void Application::LoadMap(const std::vector<std::vector<int32_t>> &map, const std::map<int32_t, std::string> catMap)
{
	m_Cells.clear();
	m_Cells.resize(m_NumberOfCells * m_NumberOfCells);
	...
	const size_t colorsSize = sizeof(colors) / sizeof(sf::Color);

	int index = 0;
	m_Categories.clear();
	for (auto &cat : catMap)
	{
		m_Categories.insert({cat.second, {colors[index++ % colorsSize], cat.first}});
	}
}

void Application::LoadMap(const std::string &filePath)
{
	std::vector<std::vector<int32_t>> readMap;
	std::map<int32_t, std::string> catMap;
	catMap.insert({-1, "DEFAULT"});
	int32_t categoryNumber = 0;
	...
	while (!inputFile.eof())
	{
		...
		while (!ss.eof())
		{
			...
			if (value < -1)
			{
				if (auto it = catMap.find(value); it == catMap.end())
					catMap.insert({value, "Category_" + std::to_string(categoryNumber++)});
			}
			temp.push_back(value);
		}

		size = temp.size();
		readMap.push_back(temp);
	}
	inputFile.close();
	this->LoadMap(readMap, catMap);
}